...
def show_channels_table(channel_name):
    ...
    df = pd.DataFrame(data, columns=["Channel_Name","Channel_Id","Subscribers","Views",  "Total_Videos", "Channel_Description","Playlist_Id","Thumbnail"])
    st.write(df)
...
def update_channel_details(channel_id):
    ch_details = get_channel_info(channel_id)
    pl_details = get_playlist_details(channel_id)
    vi_ids = get_videos_ids(channel_id)
    vi_details = get_video_info(vi_ids)
    ...
    update_channel_query = '''UPDATE channels SET
                              Channel_Name = %s,
                              Subscribers = %s,
                              Views = %s,
                              Total_Videos = %s,
                              ...
                              Thumbnail = %s
                              WHERE Channel_Id = %s'''
    cursor.execute(update_channel_query, (
        ch_details['Channel_Name'], ch_details['Subscribers'], ch_details['Views'], ch_details['Total_Videos'],
        ch_details['Channel_Description'], ch_details['Playlist_Id'], ch_details['Thumbnail'], ch_details['Channel_Id']
    ))

    # Update playlist details
    update_playlist_query = '''UPDATE playlists SET
                               Title = %s,
                               Channel_Id = %s,
                               Channel_Name = %s,
                               PublishedAt = %s,
                               ...
                               WHERE Playlist_Id = %s'''
    for pl in pl_details:
        cursor.execute(update_playlist_query, (
            pl['Title'], pl['Channel_Id'], pl['Channel_Name'], pl['PublishedAt'], pl['Video_Count'], pl['Playlist_Id']
        ))

    # Update video details
    update_video_query = '''UPDATE videos SET
                            Channel_Name = %s,
                            Channel_Id = %s,
                            Title = %s,
                            Tags = %s,
                            ...
                            WHERE Video_Id = %s'''
    for vi in vi_details:
        cursor.execute(update_video_query, (
            vi['Channel_Name'], vi['Channel_Id'], vi['Title'], vi['Tags'], vi['Thumbnail'], vi['Description'],
            vi['Published_Date'], vi['Duration'], vi['Views'], vi['Likes'], vi['Comments'], vi['Favorite_Count'],
            vi['Definition'], vi['Caption_Status'], vi['Video_Id']
        ))

    # Update comment details
    update_comment_query = '''UPDATE comments SET
                              Video_Id = %s,
                              Comment_Text = %s,
                              Comment_Author = %s,
                              Comment_Published = %s
    ...
    for com in com_details:
        cursor.execute(update_comment_query, (
            com['Video_Id'], com['Comment_Text'], com['Comment_Author'], com['Comment_Published'], com['Comment_Id']
        ))
...
def for_choice_home():
    st.write("Welcome to the YouTube Data Harvesting app. Navigate to the Search page to begin.")
...
def for_choice_Add_Remove():
    ...
    if st.button("Get Data"):
        ...
        if existing_channel:
            result = update_channel_details(channel_id)
            st.success("Channel details updated successfully.")
        else:
...
def for_choice_Analysis():
    ...
    if question == "1. All the videos and the channel name":
        ...
        # Bar plot for channels with most number of videos
        fig, ax = plt.subplots()
        sns.barplot(x="No of Videos", y="Channel Name", data=df2.head(10), ax=ax)
        ax.set_title("Top 10 Channels with Most Number of Videos")
        st.pyplot(fig)
        ...
        # Bar plot for 10 most viewed videos
        fig, ax = plt.subplots()
        sns.barplot(x="Views", y="Video Title", data=df3, ax=ax)
        ...
        t4 = cursor.fetchall()
        df4 = pd.DataFrame(t4, columns=["No of Comments", "Video Title","Channelname"])
        st.write(df4)
        ...
        # Bar plot for videos with highest likes
        fig, ax = plt.subplots()
        sns.barplot(x="Like Count", y="Video Title", data=df5.head(10), ax=ax)
        ax.set_title("Top 10 Videos with Most Likes")
        st.pyplot(fig)

    elif question == "6. Likes of all videos":
        query6 = '''SELECT likes AS likecount, title AS videotitle,channel_name AS channelname FROM videos order by channel_name asc'''
        cursor.execute(query6)
        t6 = cursor.fetchall()
        df6 = pd.DataFrame(t6, columns=["Like Count", "Video Title","Channelname"])
        st.write(df6)
        ...
        # Bar plot for views of each channel
        fig, ax = plt.subplots()
        sns.barplot(x="Total Views", y="Channel Name", data=df7, ax=ax)
        ax.set_title("Total Views of Each Channel")
        st.pyplot(fig)
        ...
        # Count plot for videos published in 2022
        fig, ax = plt.subplots()
        sns.countplot(y="Channel Name", data=df8, ax=ax)
        ax.set_title("Videos Published in 2022 by Channel")
        st.pyplot(fig)
        ...
        t9 = cursor.fetchall()
        df9 = pd.DataFrame(t9, columns=["Channel Name", "Average Duration(in mins)"])
        df9["Average Duration(in mins)"] = df9["Average Duration(in mins)"].apply(lambda x: x.total_seconds() / 60).round(2)
        st.write(df9)

        fig, ax = plt.subplots()
        sns.barplot(x="Average Duration(in mins)" ,y="Channel Name", data=df9, ax=ax)
        st.pyplot(fig)
        ...
        # Bar plot for videos with highest number of comments
        fig, ax = plt.subplots()
        sns.barplot(x="Comments", y="Video Title", data=df10.head(10), ax=ax)
